...
const App: React.FC = () => {
  ...
  // Implement handleProcessIntake for manual form submission
  ...
  const downloadMatchReport = () => {
    ...
    const headers = ["Employer", "Size", "Score", "Segment", "Website", "Phone", "Email", "Rationale"];
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden text-slate-900 font-sans">
          ...
          <div className={`p-6 border-b border-slate-800 flex ${!isActuallyExpanded ? 'flex-col items-center gap-4' : 'items-center justify-between'}`}>
            <div className={`flex items-center gap-2 transition-all duration-300 ${!isActuallyExpanded ? 'opacity-0 w-0 h-0 pointer-events-none' : 'opacity-100 w-auto'}`}>
              ...
              <div className="flex flex-col">
                ...
                <p className="text-[8px] font-bold text-slate-500 uppercase tracking-wider mt-0.5 whitespace-nowrap">Property of EdgefactoryAI.com</p>
              </div>
            </div>
      ...
      <main ref={mainRef} className={`flex-1 relative flex flex-col bg-slate-50 h-[100dvh] lg:h-screen overscroll-behavior-y-contain no-scrollbar ${['chat', 'terms', 'docs', 'support'].includes(step) ? 'overflow-hidden' : 'overflow-y-auto'}`}>
            ...
            <header className="sticky top-0 z-[70] w-full px-8 py-4 flex justify-between items-center transition-all bg-slate-900/60 backdrop-blur-xl border-b border-white/5">
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[8px] font-bold text-slate-500 uppercase tracking-widest mt-1">Property of EdgefactoryAI.com</span>
                </div>
              </div>
              ...
              <div className="max-w-6xl mx-auto relative z-10 text-center space-y-10 md:space-y-14">
                ...
                <div className="max-w-2xl mx-auto w-full pt-6 animate-in fade-in slide-in-from-bottom-16 duration-700 delay-300 flex justify-center">
                   <button
                    type="button"
                    onClick={() => setStep('intake')}
                    className="group relative inline-flex items-center gap-4 px-8 py-5 bg-blue-600 text-white rounded-2xl font-black text-xl tracking-tight hover:bg-blue-500 transition-all shadow-[0_0_40px_-10px_rgba(37,99,235,0.5)] hover:shadow-[0_0_60px_-15px_rgba(37,99,235,0.6)] hover:scale-105 active:scale-95"
                   >
        ...
        <div 
          className={`w-full max-w-6xl mx-auto ${['landing'].includes(step) ? 'hidden' : 'flex-1 flex flex-col min-h-0'} ${!['chat', 'terms', 'docs', 'support'].includes(step) ? 'pb-44 md:pb-24 px-4 md:px-8 pt-4 md:pt-8' : 'pb-0 overflow-hidden'}`}
          style={!['chat', 'terms', 'docs', 'support'].includes(step) ? { paddingBottom: 'calc(12rem + env(safe-area-inset-bottom))' } : {}}
        >
              ...
              <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200">
                ...
                <form onSubmit={handleProcessIntake} className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6 md:gap-y-8">
                  <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormField 
                      label="Workforce Title/Description" 
                      example="Advanced Manufacturing Technician" 
                      value={form.title} 
                      onChange={(v: string) => updateField('title', v)} 
                      error={errors.title} 
                      tooltip="Enter the official or working name of your program. This helps E^3 identify relevant market trends."
                    />
                    <FormField 
                  ...
                  <div className="md:col-span-2 border-t border-slate-100 pt-8 mt-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <FormField 
                        label="Current Employer Partners (Exclusions)" 
                        example="Acme Corp, Global Industries, TechFlow" 
                        value={form.currentPartners} 
                        onChange={(v: string) => updateField('currentPartners', v)} 
                        tooltip="List companies you already work with. E^3 will exclude these from new discovery results to avoid duplicate outreach."
                  ...
                  <div className="md:col-span-2 space-y-6 pt-6 pb-16">
                    ...
                    <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white px-6 py-4 rounded-xl font-bold transition-all shadow-lg active:scale-95 outline-none">Submit Profile</button>
                  </div>
                </form>
              </div>
            </div>
              ...
              {activeModal.type === 'followups' && (
                <>
                  <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex justify-between items-center mb-4"><span className="text-blue-600 uppercase tracking-widest text-[10px] font-black">Primary Email</span><ActionButtons text={outreachData[activeModal.employer].primaryEmail} label="Primary Email" onEmail={() => handleEmailExport(matches.find(m => m.name === activeModal.employer)?.contactEmail || '', outreachData[activeModal.employer].subjectLines?.[0] || 'Opportunity', outreachData[activeModal.employer].primaryEmail)} /></div>
                  ...
                  {outreachData[activeModal.employer].followUps.map((text, i) => (
                    <div key={i} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                      <div className="flex justify-between items-center mb-4"><span className="text-blue-600 uppercase tracking-widest text-[10px] font-black">Follow-up {i + 1}</span><ActionButtons text={text} label={`Follow-up ${i + 1}`} onEmail={() => handleEmailExport(matches.find(m => m.name === activeModal.employer)?.contactEmail || '', `Re: ${outreachData[activeModal.employer].subjectLines?.[0] || 'Opportunity'}`, text)} /></div>
